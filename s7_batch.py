#!/usr/bin/env python
"""System 7 — launch TOTAL classificatorias one at a time (sequential, no concurrency).
...
dashboard's /api/run/batch as a tracked job.
    ...
    s7_batch.py TOTAL STRAT ENGINE TAG

Each match runs one at a time — no parallel Evals.
"""
import os
import random
import sys
...
def main(argv):
    ...
    strat, engine, tag = argv[1], argv[2], argv[3]
    db = os.environ.get("S7_STATS_DB", "")
    print("[batch] TOTAL=%d STRAT=%s ENGINE=%s TAG=%s (sequential, 1-at-a-time)" % (total, strat, engine, tag), flush=True)
